...
namespace AssetUsageDetectorNamespace
{
    ...
    public enum PathDrawingMode { Full, ShortRelevantParts, Shortest ,Free};
    ...
    public class ReferenceHolder : ISerializationCallbackReceiver
    {
        ...
        /// <returns></returns>
        public List<Object> GetNoReferenceDatas(List<Object> objs)
        {
            List<Object> selectObjs = new List<Object>(objs.Count);//选中对比
            selectObjs.AddRange(objs);
            for(int i=0;i<references.Count;i++)//遍历依赖数据
            {
                for(int j=0; j< selectObjs.Count; j++)//遍历选中数据
                {
                    //遍历选中 ，检查是否在资源中被依赖，被依赖则可剔除
                    if (references[i].CheckReferenceContain(selectObjs[j]))
                    {
                        selectObjs.RemoveAt(j);
                        j--;
                    }
                }
    ...
    public class ReferenceNode
    {
        ...
        /// <returns></returns>
        public bool CheckReferenceContain(Object obj)
        {
            if (UnityObject != null && UnityObject.Equals(obj))
                return true;
            for(int i=0;i<links.Count;i++)
            {
                if (links[i].targetNode.CheckReferenceContain(obj))
                    return true;
            }
            return false;
        }
    ...
    public class AssetUsageDetector : EditorWindow
    {
        ...
        private List<ReferenceHolder> searchResult = new List<ReferenceHolder>(); // Overall search results//目前发现，所有的搜索数据都放在searchResult[0]中，该队列只添加了一次
        private List<Object> freeAssets = new List<Object>();
        ...
        void OnGUI()
        {
            ...
            else if (currentPhase == Phase.Complete)
            {
                ...
                GUI.color = c;
                GUILayout.Space(10);
                if (searchResult.Count == 0 && freeAssets.Count == 0)
                {
                ...
                else
                {
                    ...
                    GUILayout.BeginHorizontal();
                    GUILayout.Space(35);
                    if (EditorGUILayout.ToggleLeft("show free assets", pathDrawingMode == PathDrawingMode.Free))
                        pathDrawingMode = PathDrawingMode.Free;
                    GUILayout.EndHorizontal();
                    // Tooltip gets its value in ReferenceHolder.DrawOnGUI function
                    tooltip = null;
                    if (pathDrawingMode != PathDrawingMode.Free)
                    {
                        for (int i = 0; i < searchResult.Count; i++)
                    ...
                    {
                        //绘制未被依赖的数据
                        for(int i=0;i<freeAssets.Count;i++)
                        {
                            string title = String.Format("{0} : {1}", freeAssets[i].name, AssetDatabase.GetAssetPath(freeAssets[i]));
                            if (GUILayout.Button(title, AssetUsageDetector.BoxGUIStyle, AssetUsageDetector.GL_EXPAND_HEIGHT))
                            {
                                freeAssets[i].SelectInEditor();
                            }
                        }
                    }
                    if (tooltip != null)
        ...
        private bool ExposeSearchedAssets()
        {
            ...
            GUILayout.Label("建议一次不要添加太多查找对象，OnGUI界面在数据元素过多时会有明显卡顿。建议单次查询100条左右");
            GUILayout.BeginHorizontal();
            ...
            if (GUI.enabled)
            {
                    ...
                    if (ev.type == EventType.DragPerform)
                    {
                        ...
                        if (draggedObjects.Length > 0)
                        {
                            for (int i = 0; i < draggedObjects.Length; i++)
                            {
                                ...
                                {
                                    hasChanged = true;
                                    //create 2018/12/7 @zhangyang 添加拖拽文件夹直接填入文件夹下所有非文件夹内容
                                    //拖拽至 UI界面 Asset(s): 行。如果想添加一个文件夹请 按 "+"后拖拽至空窗
                                    string path = AssetDatabase.GetAssetPath(draggedObjects[i]);
                                    string extName = System.IO.Path.GetExtension(path);
                                    if(extName.CompareTo("") == 0)//此处判断是否为文件夹不严谨。文件夹名为x.y则文件夹后缀名为y
                                    {
                                        List<Object> subObjs = EditorHelper.LoadAllAssetsDir<Object>(path, "");//获取全部子文件
                                        for (int j = 0; j < subObjs.Count; j++)
                                        {
                                            string subPath = AssetDatabase.GetAssetPath(subObjs[j]);
                                            string subExtName = System.IO.Path.GetExtension(subPath);
                                            if (subExtName.CompareTo("") != 0)//判断是否为文件夹，与循环外部有同样不严谨的地方
                                                assetsToSearch.Add(subObjs[j]);
                                        }
                                    }
                                    else
                                    {
                ...
                if (GUILayout.Button("Clear", GL_WIDTH_100))
                    assetsToSearch.Clear();
            }
            GUILayout.EndHorizontal();
        ...
        private void ExecuteQuery()
        {
            ...
            for (int i = 0; i < assetsToSearch.Count; i++)//遍历查找目标，获取查找类型
            {
            ...
            foreach (Object asset in assetsSet)//遍历资源类型，设置搜索类型
            {
            ...
            HashSet<string> scenesToSearch = new HashSet<string>();//添加场景查询数据
            ...
            if (searchInAssetsFolder && searchedType != SearchedType.SceneObject)
            {
                ...
                string[] pathsToAssets = AssetDatabase.FindAssets("t:GameObject");//获得所有 GameObject 路径
                ...
                pathsToAssets = AssetDatabase.FindAssets("t:ScriptableObject");//获得所有 ScriptableObject 路径
                for (int i = 0; i < pathsToAssets.Length; i++)
                ...
                {
                    pathsToAssets = AssetDatabase.FindAssets("t:Material");//获得所有 Material 路径
                    for (int i = 0; i < pathsToAssets.Length; i++)
                ...
                pathsToAssets = AssetDatabase.FindAssets("t:AnimationClip");//获得所有 AnimationClip 路径
                for (int i = 0; i < pathsToAssets.Length; i++)
                ...
                pathsToAssets = AssetDatabase.FindAssets("t:RuntimeAnimatorController");//获得所有 RuntimeAnimatorController 路径
                for (int i = 0; i < pathsToAssets.Length; i++)
            ...
            //清空自由数据
            freeAssets.Clear();
            if (searchResult.Count > 0)//查找数据为空，则所有查找目标都为自由数据
            {
                for (int i = 0; i < searchResult.Count; i++)//其实只有一层
                {
                    freeAssets.AddRange(searchResult[i].GetNoReferenceDatas(assetsToSearch));//获取自由数据
                }
            }
            else
            {
                freeAssets.AddRange(assetsToSearch);
            }
